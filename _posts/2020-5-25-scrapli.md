...
There is a Python library that I have been looking at recently called scrapli. This is a Python library that you can use for screen scraping. Working with it is very similar to working with Netmiko. What lead me to checking out this package is the fact that it is easy to tap into ssh2-python. This can offer quite some benefits in terms of speed and CPU utilization. This can come in handy when you want to scrape many devices at the same time.  


### Scrapli

Scrapli can be found <a href="https://github.com/carlmontanari/scrapli" target="_blank">here</a><br>. The <b>README.md</b> is, in my opinion, pretty solid. It has a good decription of what Scrapli is and how you can use it.
...
With this example, the Scrapli will log in to the device using the SSH binary on your system. You can choose several other transport mechanisms in addition to the default. The other options include paramiko, ssh2-python and telnetlib. 
...
Using scrapli in Salt can save a lot of CPU. It is not something that will fix the (insane) memmory requirements that come with running a large number of proxies though using Salt. However, Salt is working on a delta proxy. This delta proxy will help drive down the memory usage considerably. The delta proxy together with scrapli look like something that will make the management of devices that have no API using Salt a lot better. 


Ps, another solution to the memmory issue and burden that comes with managing many proxies would be looking into <a href="https://github.com/mirceaulinic/salt-sproxy" target="_blank">salt-sproxy</a>. Things will not happen in near parallel as you do not use the message bus, but the upside is that you have no more proxy minions to manage.

https://github.com/saidvandeklundert/saidvandeklundert.github.io/blob/scrapli_salt/_posts/2020-5-25-scrapli.md