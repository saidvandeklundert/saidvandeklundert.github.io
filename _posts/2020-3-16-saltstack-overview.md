...
- **External pillar**: Salt can be configured to retrieve data from an external sytem. An example would be where someone uses Hashicorp Vault to store sensitive information. The Salt master can be made to retrieve all passwords/API tokes from Vault when building the pillar data for it’s minions.
