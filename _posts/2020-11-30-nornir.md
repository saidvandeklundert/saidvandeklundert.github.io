...
Over the last few weeks, I have been working with Nornir. Nornir is described as a 'Pluggable multi-threaded framework with inventory management to help operate collections of devices'. I looked into it hoping to find something lightweight that would just 'get the job done'.
...
Nornir let's you run tasks against a collection of devices that can be organized in it's inventory. Nornir is written in Pyton. And not only is Nornir written in Python, everything you do in Nornir is done in Python. This means that instead of working with a DSL, you express yourself in Python. 

In Nornir, you can work with everything that is available in the Python ecosystem simply by pip installing it. And not only can you make use of all the usual suspects, you are not bound or limited in any way from using whatever other packages you are fond of. 
