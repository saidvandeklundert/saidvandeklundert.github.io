...
In this article, I will first show you examples on how you can use the SaltStack to send a CLI command to a device. After this, I’ll cover an example where we use a single custom execution module function to pass a command to devices managed by a `netmiko`, `napalm` or `junos` proxy minion.
